...
### 2. 长度与 Token 消耗分布
基于真实 Tokenizer (`Qwen3-0.6B`) 编码计算（无截断处理），各核心字段的 Token 消耗详细分位数分布如下：

| 字段 (Field) | 均值 (Mean) | 中位数 (Median) | P5 | P10 | P25 | P75 | P90 | P95 | P99 | 最大值 (Max) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Prompt** | 18 | 17 | 12 | 13 | 15 | 20 | 22 | 24 | 28 | 48 |
| **Reasoning** | 1141 | 1134 | 933 | 973 | 1047 | 1228 | 1318 | 1376 | 1489 | 2070 |
| **Response** | 2479 | 2425 | 1561 | 1735 | 2045 | 2850 | 3284 | 3573 | 4188 | 6554 |

**核心结论：**
* **Prompt 极短**：用户指令高度凝练（P99 仅 28 tokens）。
* **Reasoning 充足**：绝大多数样本具备 1000 左右 token 的思考长度，能够支撑高质量的思维链（CoT）微调。
* **总长度安全水位**：数据集最大总长度（包含 30 tokens 特殊模板开销）为 **7859 tokens**，完全处于模型 8192 的安全上下文窗口内，无超长截断风险。
...
*图 3：拼接模板后的总 Token 长度分布*

> 💡 **数据资产获取说明：如何获取 profile_data.json？**  
> 本报告中提及的所有细粒度统计指标（包括详细的分位数表、相似度敏感性矩阵，以及用于后续清洗的具体异常样本索引黑名单），**无需手动统计**。只需在终端执行 `python 01_data_profiling.py`，脚本计算完成后会自动在 `data/` 目录下生成一个包含完整全量特征的 `profile_data.json` 文件。后续的清洗脚本（`02_data_cleaning.py`）将直接读取该文件实现秒级 O(1) 过滤。
...
> 在画像中我们发现，UIGEN-T2 作为 UI 垂直数据集，其指令词汇高度集中。这导致 Prompt 之间存在天然的高语义相似度（如：“设计登录页”与“设计注册页”的向量距离极近，但对应代码完全不同）。根据全量数据分位数统计（P5 = 0.849），**若盲目将阈值降至通用的 0.85，将导致高达 95% 的数据被判定为重复而遭到严重误杀**。因此，我们将 Semantic 阈值保守地定在 0.99，仅用于过滤纯同义词替换，而将主要套壳模板的清理交由对结构敏感的 MinHash (R6) 处理，以此保障微调数据的细粒度辨识力。
...
1. 训练步数极短，未触发“同质化惩罚”
在当前的实验中，训练步数极短（Batch Size = 8、Steps = 1000，仅跑了 0.3 个 Epoch），模型只遍历了约 8,000 个样本，因此未触发“同质化惩罚”。根据我们在画像脚本中的探查经验：仅随机抽取 5,000 个样本时，相似度极低；只有全量 41k 交叉对比时，才会暴露出 34% 的重复率。这说明在短程随机采样中，Raw 组根本还没来得及频繁撞见那些“冗余模板”。由于两组数据当前提供给模型的有效信息量差异不大，数据同质化的毒性尚未发作，因而收敛轨迹完全一致。如果我们将有效 Batch Size 拉大到 32及以上，或者直接解除 Steps 限制，让模型完整跑到 2-3 个 Epoch，随着冗余数据的不断重现，Raw 组潜在冗余模板的“过拟合惩罚”或许会体现在 Validation Loss 的反弹上。

2. 垂直任务天然同质化，抹平了早期差异
前端 UI 生成任务的指令天然存在高度“单一性”。这也是为什么在画像阶段，即使我们将 Semantic 语义去重的阈值拉到极其严苛的 0.99，依然能拦截 1,894 条数据（占 4.60%）；而基于字面的 MinHash 更是拦截了 14,001 条套壳模板（占 34.01%），两者去重后的 Union（并集）为 14,097 条（占 34.24%）。由于任务pormpt本身同质化严重，模型前期的核心瓶颈在于掌握基础的 HTML/Tailwind 语法规则，而非细粒度的多样性区分。在这个初步对齐阶段，无论是 Clean 还是 Raw 数据，都能提供充足的基础语法信息，模型双双迅速触及当前的收敛下限（Loss ≈ 1.425），导致曲线未产生分化。
